...
package object rtm {
  ...
  val pingFailSize: Int = config.getInt("slack.rtm_api.ping_fail_size")
  ...
  final case class RtmFailedException(message: String, subscribers: Set[ActorRef], cause: Throwable = None.orNull)
    extends Exception(message, cause)
}
